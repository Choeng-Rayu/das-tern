...
/// Displays a generated connection token as a QR code + text code.
/// Patient shows this to their doctor OR family caregiver to initiate a connection.
/// The connection type is determined by the role of whoever scans it:
/// - DOCTOR scans -> doctor connection (limited access, no medication visibility)
/// - FAMILY_MEMBER scans -> family connection (permission level from token)
class PatientDoctorTokenScreen extends StatefulWidget {
  const PatientDoctorTokenScreen({super.key});

  @override
  State<PatientDoctorTokenScreen> createState() =>
      _PatientDoctorTokenScreenState();
}

class _PatientDoctorTokenScreenState extends State<PatientDoctorTokenScreen> {
  ...
  Future<void> _generateToken() async {
    ...
    try {
      ...
      // Generate a generic connection token with ALLOWED permission
      // The backend will determine the connection type based on who scans it:
      // - If a DOCTOR scans it, creates a doctor connection (limited access)
      // - If a FAMILY_MEMBER scans it, creates a family connection (full access)
      final result = await provider.generateToken('ALLOWED');
    ...
    return SingleChildScrollView(
      child: Column(
        children: [
          ...
          // Instructions for connection (works for both doctor and family)
          AppCard(
            child: Column(
              ...
              children: [
                ...
                _buildStep(
                  context,
                  '1',
                  'Share this QR code or connection code with your doctor or family caregiver',
                ),
                _buildStep(
                  context,
                  '2',
                  'They will scan it to request a connection',
                ),
                _buildStep(
                  context,
                  '3',
                  'You will need to approve the connection request',
                ),
              ],
            ),
          ),
  ...
  Future<void> _shareTokenWithQr() async {
    ...
    try {
      ...
      // 2. Rasterise the widget to a ui.Image at 3x device-pixel density
      ...
      final File qrFile = await File(
        '${tempDir.path}/dastern_doctor_qr_$_token.png',
      ).create();
