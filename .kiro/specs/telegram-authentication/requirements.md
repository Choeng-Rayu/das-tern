...
The current implementation uses **Telegram Login 2.0 OIDC Authorization Code Flow with PKCE** (`oauth.telegram.org`) and server-side **ID token validation via JWKS**, not the legacy widget hash flow.

- Flutter sends `code`, `codeVerifier`, and `redirectUri` to backend `POST /auth/telegram`
- Backend exchanges `code` at Telegram token endpoint
- Backend validates `id_token` signature and claims (`iss`, `aud`, `exp`)
- Backend then finds/creates user and returns app JWT tokens
...
2. WHEN the user taps the "Continue with Telegram" button, THE Flutter_App SHALL open Telegram OAuth authorization endpoint (`https://oauth.telegram.org/auth`) with PKCE parameters
3. THE Flutter_App SHALL generate and send `state`, `code_challenge`, and `code_challenge_method=S256` in the authorization request
4. THE Flutter_App SHALL handle deep-link callback containing authorization `code` and `state`
5. THE Flutter_App SHALL support both English and Khmer localization for the button text
6. THE Flutter_App SHALL register a deep link handler for the `dastern://auth/telegram/callback` scheme
...
### Requirement 3 (Aligned): Telegram ID Token Verification

**User Story:** As a security system, I want to verify Telegram OIDC ID tokens, so that I can prevent authentication forgery

#### Acceptance Criteria

1. THE Auth_Service SHALL fetch Telegram signing keys from `https://oauth.telegram.org/.well-known/jwks.json`
2. THE Auth_Service SHALL verify `id_token` signature using the matching JWK (`kid`)
3. THE Auth_Service SHALL reject tokens with invalid signature, missing `sub`, or unsupported algorithm
4. THE Auth_Service SHALL verify `iss` equals `https://oauth.telegram.org`
5. THE Auth_Service SHALL verify `aud` equals configured `TELEGRAM_BOT_CLIENT_ID`
6. THE Auth_Service SHALL verify token expiry (`exp`) and reject expired tokens
7. THE Auth_Service SHALL log verification failures for security monitoring
...
### Requirement 4 (Aligned): OIDC Token Freshness Validation

**User Story:** As a security system, I want to reject expired Telegram OIDC tokens, so that I can prevent replay attacks

#### Acceptance Criteria

1. THE Auth_Service SHALL validate `exp` claim against current server time
2. IF the token is expired, THEN THE Auth_Service SHALL reject the request with 401 Unauthorized
3. THE Auth_Service SHALL validate `iat` format when present
4. THE Auth_Service SHALL log rejected authentication attempts with expired tokens
...
### Requirement 8 (Aligned): OAuth Request Validation

**User Story:** As a security system, I want to validate OAuth request parameters, so that I can prevent malicious authentication attempts

#### Acceptance Criteria

1. THE Auth_Service SHALL validate required parameters (`code`, `codeVerifier`, `redirectUri`)
2. IF any required parameter is missing, THEN THE Auth_Service SHALL reject the request with 400 Bad Request
3. THE Auth_Service SHALL reject token exchange failures with 401 Unauthorized
4. THE Auth_Service SHALL sanitize received identity fields before persistence
5. THE Auth_Service SHALL enforce HTTPS for auth endpoints in production
...
### Requirement 11 (Aligned): External Browser + Deep Link and Token Storage

**User Story:** As a mobile app, I want to authenticate using Telegram OAuth in browser and return via deep link, so that users can complete authentication reliably

#### Acceptance Criteria

1. THE Flutter_App SHALL launch Telegram OAuth URL in external browser
2. THE Flutter_App SHALL listen for `dastern://auth/telegram/callback`
3. THE Flutter_App SHALL validate callback `state` before sending data to backend
4. THE Flutter_App SHALL call backend `POST /auth/telegram` with `code`, `codeVerifier`, and `redirectUri`
5. THE Flutter_App SHALL store returned JWT tokens in `flutter_secure_storage`
6. IF authentication fails, THEN THE Flutter_App SHALL display an error and remain on auth screens
