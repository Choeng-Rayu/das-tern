...
export class BakongClient {
    ...
    try {
      const response = await retry(
        async () => {
          ...
          return await this.client.post('/check_transaction_by_md5', {
            md5: md5Hash,
          });
        },
        {
      ...
      logger.info('Bakong check_transaction_by_md5 response', {
        md5Hash,
      ...
      if (data.responseCode === 0 && data.data) {
        ...
        logger.info('Payment status: PENDING/UNPAID', {
          md5Hash,
          responseCode: data.responseCode,
        });
        return {
  ...
  async generateDeeplink(
    ...
    try {
      const response = await this.client.post('/generate_deeplink_by_qr', {
        ...
        sourceInfo: {
          appIconUrl:
            options?.appIconUrl || 'https://bakong.nbc.gov.kh/images/logo.svg',
  ...
  async bulkCheckPayments(md5Hashes: string[]): Promise<PaymentStatus[]> {
    ...
    try {
      ...
      const results: PaymentStatus[] = (data.data || []).map(
        (item: any) =>
          ({
            md5Hash: item.md5,
