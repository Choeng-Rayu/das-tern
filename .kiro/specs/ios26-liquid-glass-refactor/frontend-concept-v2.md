# Frontend Concept (v2) — Das Tern Liquid Glass

> ⚠️ **2026-05-26 — This file is a draft snapshot kept for reference.** The canonical v2 frontend visual layer now lives at **[`/.kiro/specs-v2-flutter-supabase/10-frontend-liquid-glass/`](../../specs-v2-flutter-supabase/10-frontend-liquid-glass/)** with full `requirements.md`, `design.md`, and `tasks.md`. Update the v2 spec set, not this file.

> **This is the v2 frontend concept.** It supersedes the v1 RxCam-flavoured sections of the older `requirements.md`, `design.md`, and `tasks.md` in this folder.
>
> v2 architecture lives at [`/.kiro/specs-v2-flutter-supabase/`](../specs-v2-flutter-supabase/). When this concept and the v2 spec set disagree, **v2 wins**. This document focuses on the visual and interaction language; v2 owns the data, security, and platform decisions.

---

## 1. North star

Das Tern v2 is a Flutter app for patients and doctors managing medication. It runs on a single codebase (Android first, iOS-ready), backed by Supabase. We rebuild the visual language on Apple's iOS 26 **Liquid Glass** vocabulary so the app feels alive, calm, and trustworthy — even when the user is anxious about a missed dose.
...
|---|---|---|
| Color/typography/spacing tokens | [`09-design-system-localization`](../specs-v2-flutter-supabase/09-design-system-localization/) | **Inherits** them. The Liquid Glass surfaces are styled exclusively via these tokens. |
...
A reader who only opens this folder should land on this file first, then the v2 README, then the design token spec.
...
- Both controllers `repeat(reverse: true)` and respect `MediaQuery.disableAnimations`.
...
| `AppMeshBackground` | `Stack` + `CustomPaint` + `AnimationController × 3` | Always at the root of `AppScaffold`. |
...
Four tabs (no Connections tab — doctors get patient list), plus the same QR FAB anchored to the AppBar trailing area:
...
- [ ] `09-design-system-localization` ships `GlassTokens` extension on both themes.
- [ ] `lib/shared/widgets/effects/frosted_surface.dart` and `lib/shared/widgets/glass/*.dart` implemented per § 7.
- [ ] `AppScaffold` integrates mesh + header + role-aware nav.
- [ ] Patient and Doctor home routes wired to the new shell.
- [ ] All v2 feature screens (`02–08`) pass through `FrostedSurface` for elevated content (no raw `Card` left).
- [ ] CI lint rule blocks `BackdropFilter` without a key outside `frosted_surface.dart`.
- [ ] Golden tests cover light/dark for every glass widget.
- [ ] Performance probe enforces frame budget on the lowest-tier device in the test matrix.
- [ ] Accessibility audit signs off contrast, motion, and tap targets.

## 10. Out of scope

- Custom shaders. iOS 26's true Liquid Glass uses Metal shaders for the wet/specular look; Flutter's `BackdropFilter` plus a tinted gradient is a faithful approximation, not a 1:1 render. We accept the visual gap.
- Live wallpaper-aware tinting (would require platform channels). Mesh orbs are sufficient.
- Watchaccess / wearable companion. Out of v2 MVP; the design tokens still apply when added.
