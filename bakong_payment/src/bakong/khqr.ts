...
export class BakongKHQR {
  ...
  static createQR(params: KHQRParams): string {
    ...
    // Tag 99 - Timestamp (dynamic KHQR)
    // Must contain sub-tags:
    //   00 = creation timestamp (13-digit ms)
    //   01 = expiration timestamp (13-digit ms)
    // Previous format only included creation timestamp, which causes
    // bank apps to treat the QR as invalid/expired.
    const expiryMinutes = Number(process.env.BAKONG_QR_EXPIRY_MINUTES || 15);
    ...
    const timestampField =
      this.formatTag('00', creationTimestamp) +
      this.formatTag('01', expirationTimestamp);
    qrData.push(this.formatTag('99', timestampField));
   ...
   */
  static generateQRImage(
    qrCode: string,
    options: ImageOptions = { format: 'png', size: 256 },
  ): string {
