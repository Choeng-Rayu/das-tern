...
import '../../features/prescriptions/presentation/pages/medication_form_page.dart';
...
import '../../features/prescriptions/presentation/pages/version_history_page.dart';
...
class AppRoute {
  ...
  static const String patientPrescriptionNew = '/patient/prescriptions/new';
...
final Provider<GoRouter> appRouterProvider = Provider<GoRouter>((Ref ref) {
  return GoRouter(
    ...
    routes: <RouteBase>[
      // ── Auth ──────────────────────────────────────────────────────────
      GoRoute(path: AppRoute.welcome, builder: (_, _) => const WelcomePage()),
      GoRoute(path: AppRoute.methodChooser, builder: (_, _) => const MethodChooserPage()),
      GoRoute(path: AppRoute.signIn, builder: (_, _) => const SignInPage()),
      GoRoute(path: AppRoute.signUp, builder: (_, _) => const SignUpCredentialsPage()),
      GoRoute(path: AppRoute.profileBootstrap, builder: (_, _) => const ProfileBootstrapPage()),
      GoRoute(
        path: '/phone-otp',
        builder: (_, state) => PhoneOtpPage(
          phone: state.extra as String? ?? '',
        ),
      ),
      GoRoute(path: AppRoute.forgotPassword, builder: (_, _) => const ForgotPasswordPage()),
      GoRoute(path: AppRoute.resetPassword, builder: (_, _) => const ResetPasswordPage()),
      GoRoute(path: AppRoute.verifyPractice, builder: (_, _) => const VerifyPracticePage()),
      // ── App shell ─────────────────────────────────────────────────────
      GoRoute(path: AppRoute.home, builder: (_, _) => const HomePage()),
      GoRoute(path: AppRoute.patientHome, builder: (_, _) => const HomePage()),
      GoRoute(path: AppRoute.doctorHome, builder: (_, _) => const HomePage()),
      // ── Prescriptions ─────────────────────────────────────────────────
      GoRoute(path: AppRoute.patientPrescriptions, builder: (_, _) => const PrescriptionListPage()),
      GoRoute(path: AppRoute.patientPrescriptionNew, builder: (_, _) => const CreatePrescriptionPage()),
      GoRoute(
      ...
      GoRoute(
        path: '/patient/prescriptions/:id/medications/new',
        builder: (_, state) => MedicationFormPage(
          prescriptionId: state.pathParameters['id']!,
        ),
      ),
      GoRoute(
        path: '/patient/prescriptions/:id/history',
        builder: (_, state) => VersionHistoryPage(
          prescriptionId: state.pathParameters['id']!,
        ),
      ),
      // ── Settings ──────────────────────────────────────────────────────
      GoRoute(
        ...
        routes: <RouteBase>[
          GoRoute(path: 'appearance', builder: (_, _) => const AppearanceSettingsPage()),
